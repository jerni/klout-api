<?php
namespace Klout\ApiBundle\Service;
...
class KloutAPIv2 {
	...
	public function  __construct(ContainerInterface $container){
		...
		$url = $this->container->getParameter('klout_api.url');
		$version = $this->container->getParameter('klout_api.version');
		$this->BaseUrl = "{$url}$version/";
		$this->KloutKey = $this->container->getParameter('klout_api.key');
	}
...
/******************************************************/
	...
	public function getKloutID(){
		...
		if(!is_numeric($this->user)){
			// twitter screenname
			$kloutid = $this->KloutIDLookupByName('twitter',$this->user);
			if($kloutid){
				return $kloutid;
			}
		} else {
			//gplus
			$kloutid = $this->KloutIDLookupByID('gp',$this->user);
			if($kloutid){
				return $kloutid;
			}
			
			//twitter id
			$kloutid = $this->KloutIDLookupByID('tw',$this->user);
			if($kloutid){
				return $kloutid;
			}
		}
